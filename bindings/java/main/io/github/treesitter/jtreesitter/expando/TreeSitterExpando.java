package io.github.treesitter.jtreesitter.expando;

import java.lang.foreign.*;

public final class TreeSitterExpando {
    ...
    private static final TreeSitterExpando INSTANCE = new TreeSitterExpando();

    private final Arena arena = Arena.ofAuto();
    private volatile SymbolLookup lookup = null;

    private TreeSitterExpando() {}
    ...
    public static MemorySegment language(SymbolLookup lookup) {
        return call(lookup, "tree_sitter_expando");
    }

    private SymbolLookup findLibrary() {
        try {
            var library = System.mapLibraryName("tree-sitter-expando");
            return SymbolLookup.libraryLookup(library, arena);
        } catch (IllegalArgumentException ex1) {
            try {
                System.loadLibrary("tree-sitter-expando");
