...
class Build(build):
    def run(self):
        if path.isdir("queries"):
            dest = path.join(self.build_lib, "tree_sitter_expando", "queries")
...
setup(
    ...
    package_data={
        "tree_sitter_expando": ["*.pyi", "py.typed"],
        "tree_sitter_expando.queries": ["*.scm"],
    },
    ext_package="tree_sitter_expando",
    ext_modules=[
        Extension(
            name="_binding",
            sources=[
                "bindings/python/tree_sitter_expando/binding.c",
                "src/parser.c",
            ],
            define_macros=[
